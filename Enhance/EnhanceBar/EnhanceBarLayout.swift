...
class EnhanceBarLayout: TMBarLayout {
    ...
    private var containers = [EnhanceBarButtonContainer]()
    ...
    override func insert(buttons: [TMBarButton], at index: Int) {
        buttons.forEach { (button) in
            let container = EnhanceBarButtonContainer(for: button)
    ...
    override func remove(buttons: [TMBarButton]) {
        let containers = stackView.arrangedSubviews.compactMap({ $0 as? EnhanceBarButtonContainer })
